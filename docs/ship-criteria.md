...
2. Name it in the present tense (`README anti-cheat tagline present`).
...
SHIP-001 | README anti-cheat tagline present | grep -q "Stop AI coding agents from faking test results" README.md
...
SHIP-008 | Track A §1 alert lane module | test -f src/alerts.ts && test -f src/alert-lane.tsx && grep -qE "cheat-done|cheat_done|verify_ran_at IS NULL" src/alerts.ts
...
SHIP-019 | V1 README hero reframed around anti-cheat receipts | grep -qE "cheat|fake|fabricated|receipts" README.md
SHIP-020 | V2 demo gif checked in and referenced in README | test -s demo/intro.gif && grep -q "intro.gif" README.md
SHIP-021 | V3 cheat-detection scorecard published with results | test -s evals/results/2026-04-24-cheat-detection.md && grep -qiE "scenario|verdict|PASS|FAIL" evals/results/2026-04-24-cheat-detection.md
